...
//		doPrjReqSaveFile	(user, obj, ent);
	...
	private static Criterion reqRestrictionSearch(TaAutUser user, String searchKey, Set<Integer> stats) {
		...
		if (searchKey!=null) {
			...
			cri = 	 Restrictions.and	(
					cri, Restrictions.or(
						Restrictions.like(ViPerPersonDyn.ATT_T_NAME_01	, searchKey), 
						Restrictions.like(ViPerPersonDyn.ATT_T_NAME_02	, searchKey), 
						Restrictions.like(ViPerPersonDyn.ATT_T_NAME_03	, searchKey), 
						Restrictions.like(ViPerPersonDyn.ATT_T_CODE_01	, searchKey), 
						Restrictions.like(ViPerPersonDyn.ATT_T_CODE_02	, searchKey), 
						Restrictions.like(ViPerPersonDyn.ATT_T_CODE_03	, searchKey),
						Restrictions.like(ViPerPersonDyn.ATT_T_CODE_07	, searchKey),//email
						Restrictions.like(ViPerPersonDyn.ATT_T_CODE_09	, searchKey))//phone 
			);
		}
	...
	private static 	ResultPagination reqLstPage(TaAutUser user,  JSONObject json, HttpServletResponse response) throws Exception  {
		...
		String keyWord 	= manId + "_" + uId + "_" + begin + "_" + number + "_" + total + "_" + searchKey + "_" + wAva + "_" + isAll;
