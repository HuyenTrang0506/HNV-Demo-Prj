...
public class TaTpyDocument extends EntityAbstract<TaTpyDocument> {
	...
	public static List<TaTpyDocument> reqTpyDocuments (Integer entType, Set<Integer> entId, Integer type01, Integer type02, Integer type03, String searchKey) throws Exception{		
		Criterion cri = Restrictions.gt("I_ID", 0);	
	...
	private static void doHideInfo (List<TaTpyDocument> lst) {
...
//				doc.reqSet(TaTpyDocument.ATT_T_INFO_02 , null); //hide to client the path real in server
	...
	public static void doBuildTpyDocuments(List listEnts, Integer parentType, Integer type01, Integer type02, String attrDocsName, boolean forced) throws Exception{
		...
		List<TaTpyDocument> docs = reqTpyDocuments(parentType, tabDocs.keySet(), type01, type02, null, null);
	...
	static {
		if(files_root_dir == null) {
			files_root_dir 		= API.reqContextParameter("PATH_TPY_DOCUMENT");
		}
...
//				}
	...
	public static List<TaTpyDocument> reqBuildAvatar(List lst, int entTyp, String colAvatarName) throws Exception {
		...
		if(!ids.isEmpty()) {
			...
			List<TaTpyDocument> avas = TaTpyDocument.reqTpyDocuments (entTyp, ids, TaTpyDocument.TYPE_01_FILE_MEDIA, TaTpyDocument.TYPE_02_FILE_IMG_AVATAR,  null, null);
